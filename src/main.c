...
void wait_for_visualizer(void) {
    ...
    g_visualizer_ready = 0;
}
int q1, q2, q3;             /* Quadrant Position (X, Y, Z) */
int old_q1=-1, old_q2=-1, old_q3=-1; /* Persistence tracking */
double s1, s2, s3;          /* Sector Position (X, Y, Z) */
int g[11][11][11];             /* The Galaxy Cube */
int z[11][11][11];             /* Scanned Map Cube */
int k[4][6];                /* Enemy Data: x, y, z, power, species_id */
int stars_pos[10][3];       /* Star positions in current quadrant */
int base_pos[3][3];         /* Base positions in current quadrant */
int planet_pos[3][5];       /* Planet positions: x, y, z, mineral_type, quantity */
int bh_pos[1][3];           /* Black Hole position */
int inventory[7];           /* 1: Dil, 2: Tri, 3: Ver, 4: Mono, 5: Iso, 6: Gases */
int species_counts[11];     /* Counts for each of the 11 species */
int k3, b3, st3, p3, bh3;   /* Current Klingons, Bases, Stars, Planets, Black Holes */
int k9, b9;                 /* Total Klingons and Bases left */
...
int corbomite_count = 0;
float system_health[8];     /* 0: Warp, 1: SRS, 2: LRS, 3: Phaser, 4: Torp, 5: Shields, 6: Transporter, 7: Life Support */
float life_support = 100.0f;
double t, t0;               /* Stardates */
int t9;                     /* Time limit */
char sC[32];                /* Condition */
char sQ[194];               /* 2D Slice for terminal (Legacy compatibility) */
char captain_name[64] = "Kirk";
char sG2[128];              /* Current Location Name */
/* Visual Effects */
int beam_targets[4][3];
int beam_count = 0;
...
void life_support_check(void) {
    if (system_health[7] >= 100.0f) {
        if (life_support < 100.0f) {
            life_support += 1.0f; /* Recovery */
            if (life_support > 100.0f) life_support = 100.0f;
        }
        return;
    }
    
    /* Consumption is proportional to damage. */
    float drain = (100.0f - system_health[7]) / 100.0f;
    if (drain > 0) {
        life_support -= drain;
        
        if (life_support < 30.0f) {
            printf(B_RED "\n*** CRITICAL LIFE SUPPORT WARNING ***\n" RESET);
            printf("Air reserves at %.1f%% and falling fast!\n", life_support);
        } else {
            printf(B_YELLOW "\n--- Life Support Notice ---\n" RESET);
            printf("Air reserves decreasing due to system damage: %.1f%%\n", life_support);
        }

        if (life_support <= 0) {
            printf(B_RED "\n[DEATH] The crew has suffocated. Mission failure.\n" RESET);
            exit(0);
        }
    }
}
...
void cleanup(void) {
    ...
    system("killall ultra_view 2>/dev/null");
}
int main(void) {
    setlocale(LC_ALL, "C");
    ...
    srand(time(NULL));
    atexit(cleanup);
    system("killall -9 ultra_view 2>/dev/null");
    ...
    if (q1 != old_q1 || q2 != old_q2 || q3 != old_q3) {
        ...
        for(i=0; i<st3; i++) {
            ...
            while (!valid && attempts < 50) {
                ...
                for (int p_idx=0; p_idx<p3; p_idx++) {
                    double d = sqrt(pow(stars_pos[i][0]-planet_pos[p_idx][0], 2) + pow(stars_pos[i][1]-planet_pos[p_idx][1], 2) + pow(stars_pos[i][2]-planet_pos[p_idx][2], 2));
                    if (d < 5.0) { valid = false; break; }
                }
                attempts++;
            }
        }
        ...
        for(i=0; i<bh3; i++) {
            ...
            while (!valid && attempts < 100) {
                ...
                valid = true;
                for (int p_idx=0; p_idx<p3; p_idx++) {
                    double d = sqrt(pow(bh_pos[i][0]-planet_pos[p_idx][0], 2) + pow(bh_pos[i][1]-planet_pos[p_idx][1], 2) + pow(bh_pos[i][2]-planet_pos[p_idx][2], 2));
                    if (d < 7.0) { valid = false; break; }
                }
                for (int s_idx=0; s_idx<st3; s_idx++) {
                    double d = sqrt(pow(bh_pos[i][0]-stars_pos[s_idx][0], 2) + pow(bh_pos[i][1]-stars_pos[s_idx][1], 2) + pow(bh_pos[i][2]-stars_pos[s_idx][2], 2));
                    if (d < 7.0) { valid = false; break; }
                }
                attempts++;
            }
...
void update_3d_view(void) {
    FILE *f = fopen("/tmp/ultra_map.tmp", "w");
    if (!f) return;
    int avg_s = (shields[0] + shields[1] + shields[2] + shields[3]) / 4;
    ...
    for (int i=0; i<beam_count; i++) {
        fprintf(f, "BEAM %d %d %d\n", beam_targets[i][0], beam_targets[i][1], beam_targets[i][2]);
    }
    beam_count = 0; /* Clear for next call */

    /* Torpedo and Explosion */
    if (cur_torp[0] > -10) fprintf(f, "TORP %f %f %f\n", cur_torp[0], cur_torp[1], cur_torp[2]);
    if (cur_boom[0] > -10) {
        fprintf(f, "BOOM %f %f %f\n", cur_boom[0], cur_boom[1], cur_boom[2]);
        cur_boom[0] = -100; /* Reset after one send */
    }
    if (cur_dismantle[0] > -10) {
        fprintf(f, "DISMANTLE %f %f %f %d\n", cur_dismantle[0], cur_dismantle[1], cur_dismantle[2], dismantle_species);
        cur_dismantle[0] = -100;
    }
...
void short_range_scan(void) {
    ...
    printf("Shields - F: %d  R: %d  T: %d  B: %d\n", shields[0], shields[1], shields[2], shields[3]);
...
void shield_control(void) {
    ...
    printf("1: Front [%d]  2: Rear [%d]  3: Top [%d]  4: Bottom [%d]\n", shields[0], shields[1], shields[2], shields[3]);
    printf("Select shield to charge (1-4) or 0 to balance: ");
    ...
    if (i == 0) {
        /* Balance */
        int total_s = shields[0] + shields[1] + shields[2] + shields[3] + amount;
        if (total_s < 0) total_s = 0;
        if (amount > e) amount = e;
        e -= amount;
        for(int j=0; j<4; j++) shields[j] = total_s / 4;
    } else if (i >= 1 && i <= 4) {
        i--; /* 0-indexed */
        if (amount > e) amount = e;
    ...
    printf("New Shield Status - F: %d  R: %d  T: %d  B: %d\n", shields[0], shields[1], shields[2], shields[3]);
}
void dock_control(void) {
    ...
    if (near_base) {
        ...
        for(int i=0; i<4; i++) shields[i] = 0; /* Shields are always lowered for docking */
        update_3d_view();
    } else {
    ...
    for (int i = 1; i <= 3; i++) {
        if (k[i][3] > 0) {
            ...
            if (fabs(dz) > fabs(dx) && fabs(dz) > fabs(dy)) {
                ...
                /* Simplified: Y is North/South. North is -Y in our trig but let's just use relative */
                s_idx = (dy < 0) ? 0 : 1; 
            }
            char *s_names[] = {"FRONT", "REAR", "TOP", "BOTTOM"};
...
void status_report(void) {
    ...
    printf("Shields - FRONT: %d  REAR: %d  TOP: %d  BOTTOM: %d\n", shields[0], shields[1], shields[2], shields[3]);
        ...
        if (opt == 1) {
            ...
            if (corbomite_count > 0) {
                ...
                if (rand()%100 > 60) {
                    ...
                    for(int i=1; i<=3; i++) if (k[i][3]>0) { k[i][0]+=2; k[i][1]+=2; }
